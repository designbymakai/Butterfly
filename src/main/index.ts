...
ipcMain.handle('fetch-openai-response', async (_, input, userApiKey) => {
  // Use the provided project key if available; otherwise, fallback to an env variable.
  const projectKey = (userApiKey && userApiKey.trim()) || process.env.PROJECT_API_KEY
  if (!projectKey) {
    throw new Error('No project API key provided.')
  }

  // If the key starts with "sk-proj-", add an extra header with your project id.
  const additionalHeaders = {}
  if (projectKey.startsWith('sk-proj-')) {
    additionalHeaders['OpenAI-Project'] = 'proj_VITpibu6Gc3MyIws7IaqtBrZ' // Replace with your actual project identifier.
  }
  try {
